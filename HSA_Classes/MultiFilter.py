...
def multifilter(multi_filters : int = 15, batch_size : int = 2000, penalty_ratio : float = .9, cutoff_distance : float = 2.5, anomaly_std_tolerance : float = 1, mf_data: np.ndarray = None, all_index_user: np.ndarray = None, pass1_model : hsa_model = None):
    for mf_n in range(multi_filters):
        ...
        for data in batch_loader:
            ...
            MF_model.set_directories(
                pass1_model.log_directory, pass1_model.results_directory
            )
            ...
            MF_model.infer(
                pass1_model.preprocessed_df, multifilter_flag=1, all_index_user=all_index_user
            )
            j += 1
            if j > 3:
                break
            pass1_model.anomaly_prediction_frequency_df.loc[
                pass1_model.anomaly_prediction_frequency_df["User DF Index"].isin(MF_model.anomaly_index_raw),
                "Anomaly Bin Count",
            ] += 1
            ...
            mix_index, mix_data, anomaly_index = (
                pass1_model.global_collect_multifilter_df(
                    ...
                    pass1_model.total_anomaly_index[: len(pass1_model.preprocessed_df.to_numpy())].astype(int),
                    mf_batch_size=9 * len(pass1_model.total_anomaly_index),
                )
            )
