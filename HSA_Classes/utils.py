...
class filter_prior_predictions:
    ...
    # self.prior_json, self.time_range_results_df, self.null_results_list = get_results_lookback(
    #     results_path, lookback_days
    # )
    # todays_results = filter_time_range_results(self.time_range_results_df, todays_df)
    # return todays_results
    ...
    def remove_predefined_ips(self, ip_key):
        ...
        if type(self.ip_key) is str:
            if self.ip_key in self.ip_df.keys():
                ...
                print(f"{self.ip_key} not in {self.ip_df.keys()}")
                sys.exit(0)
        else:
